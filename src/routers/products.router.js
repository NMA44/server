...
router.get('/products/:id', async (req, res)=>{
    ...
    if(product){
        res.status(200).json({message:'Usuario encontrado exitosamente', user:product});
    }else{
        res.status(400).json({message:`El usuario con id ${id} no existe en la base de datos.`})
    }    
